...
class WSM_Settings
{
        ...
        <div class="wrap wsm-wrap">
            ...
            <?php if (empty($all_forms)):
                ...
                <div class="notice notice-warning">
                    ...
                    <p><strong>Found potential Forminator matches:</strong>
                        <?php echo $fi_tables ? esc_html(count($fi_tables)) . ' tables' : '<em>none</em>'; ?>,
                        <?php echo $fi_pts ? esc_html(count($fi_pts)) . ' post types' : '<em>none</em>'; ?>.</p>
                    <p>Please ensure Forminator is installed and active. If the issue persists, contact your developer.</p>
                </div>
            <?php else: ?>
                <div class="wsm-settings-flex-container">
                                            ...
                                            <tr>
                                                ...
                                                <td data-label="Track?">
                                                    <input type="checkbox" class="wsm-form-checkbox" name="form_ids[]"
                                                        value="<?php echo esc_attr($form->id); ?>" <?php checked(in_array((string) $form->id, array_map('strval', $tracked_ids))); ?> />
                                                </td>
                                                <td data-label="Duplication Settings">
                                                    <div class="wsm-dup-control-wrapper" style="margin-bottom:10px;">
                                                        <label style="font-weight:700; display:flex; align-items:center; gap:8px;">
                                                            <input type="checkbox" class="wsm-track-dups-checkbox"
                                                                name="track_dup_forms[]" value="<?php echo esc_attr($form->id); ?>"
                                                                <?php checked($is_dup_tracked); ?> />
                                                            Track Duplications
                                                        </label>
                                                    </div>
                                                    <div class="wsm-duplicate-fields-container"
                                                        style="<?php echo $is_dup_tracked ? '' : 'display:none;'; ?>">
                                                        <?php if (empty($fields)): ?>
                                                            <em style="font-size: 11px; color: #666;">No entries found.</em>
                                                            <input type="text" class="wsm-duplicate-fields-input wsm-input"
                                                                name="duplicate_fields[<?php echo esc_attr($form->id); ?>]"
                                                                ...
                                                                <?php foreach ($fields as $fkey):
                                                                    ...
                                                                    <label class="wsm-field-option">
                                                                        <input type="checkbox" class="wsm-dup-check"
                                                                            value="<?php echo esc_attr($fkey); ?>" <?php checked(in_array($fkey, $dup_arr)); ?>>
                                                        ...
                                                        <div class="wsm-legacy-container">
                                                            ...
                                                            <div class="wsm-legacy-status" style="margin-bottom:8px;">
                                                                <?php if ($has_legacy): ?>
                                                                    ...
                                                                    <button type="button" class="button button-link-delete wsm-clear-legacy"
                                                                        data-fid="<?php echo esc_attr($form->id); ?>"
                                                                        style="font-size:11px; margin-left:5px;">Clear & Re-import</button>
                                                                <?php else: ?>
                                                                    <span style="font-size:12px; color:#666;">No legacy data.</span>
                                                                <?php endif; ?>
                                                            </div>

                                                            <div class="wsm-legacy-import-ui"
                                                                style="<?php echo $has_legacy ? 'display:none;' : ''; ?>">
                                                                <input type="file" class="wsm-legacy-file" accept=".csv"
                                                                    style="font-size:11px; width:100%; margin-bottom:5px;">
                                                                <div class="wsm-legacy-mapping"
                                                                    style="display:none; margin-top:10px; border-top:1px solid #e2e8f0; padding-top:12px;">
                                                                    <p
                                                                        style="margin:0 0 8px 0; font-size:11px; font-weight:700; text-transform:uppercase; color:var(--wsm-text-muted);">
                                                                        Step 2: Map CSV Columns</p>
                                                                        ...
                                                                        <label style="grid-column: span 2;">Forminator Field to Match:
                                                                            <select class="wsm-form-field-key" style="width:100%;">
                                                                                <?php foreach ($fields as $fk): ?>
                                                                                    <option value="<?php echo esc_attr($fk); ?>" <?php selected($fk, $config['match_field'] ?? ''); ?>>
                                                                                        <?php echo esc_html($fk); ?></option>
                                                                                <?php endforeach; ?>
                                                                            </select>
                                                                        </label>
                                                                    </div>
                                                                    <button type="button" class="button button-small wsm-start-import"
                                                                        data-fid="<?php echo esc_attr($form->id); ?>"
                                                                        style="margin-top:12px; width:100%;">Start Import</button>
                                                                </div>
                                                            </div>
                                                        </div>
                                    </div>
                    ...
                    <p><button type="submit" class="button button-primary">Save Settings</button>
                        <span id="wsm-settings-msg" style="margin-left:12px;font-weight:600;"></span>
                    </p>
                    </form>
                </div>
                ...
                <div class="wsm-settings-section wsm-section-tabs">
                    <h3 style="margin-top:0">2. Reorder Tabs <span style="font-size:12px;font-weight:400;color:#666;">(drag to
                            rearrange)</span></h3>
    ...
    public function get_csv_headers()
    {
        ...
        $allowed_mimes = ['text/csv', 'text/plain', 'application/csv', 'application/vnd.ms-excel', 'text/comma-separated-values'];
        ...
        if ($extension !== 'csv' || (!empty($mime_type) && !in_array($mime_type, $allowed_mimes))) {
            wp_send_json_error('Invalid file type. Please upload a valid CSV file.');
        }
    ...
    public function import_legacy()
    {
        ...
        if (!$headers || count($headers) < max($uid_col, $val_col)) {
            fclose($tmp_handle);
            wp_send_json_error('Invalid CSV structure. Mapping columns not found.');
        }
        ...
        try {
            ...
            while (($row = fgetcsv($tmp_handle)) !== false) {
                ...
                if ($uid && $normalised) {
                    $result = $wpdb->insert(WSM_TABLE_LEGACY, [
                        'form_id' => $form_id,
                        'legacy_uid' => $uid,
